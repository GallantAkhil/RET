...
app.post('/AuthReturns',function(req, res){
	...
	sql.connect(config, function(err){
		...
		request.query(`Select P.ID, P.CustomerID, P.CustomerName, P.DateTime, Status_Desc from dbo.RET_ReturnStatus INNER JOIN (Select dbo.RET_ReturnsHeader.ID, dbo.RET_ReturnsHeader.CustomerID, C.CustomerName, dbo.RET_ReturnsHeader.DateTime, dbo.RET_ReturnsHeader.Status From dbo.RET_ReturnsHeader INNER JOIN dbo.RET_Customer C ON dbo.RET_ReturnsHeader.CustomerID = C.ID where C.ManagedBy = 1 and status != 4) P ON dbo.RET_ReturnStatus.ID = P.Status`, function(err, result){
			if(err) console.log(err)
				res.end(JSON.stringify(result));
		})
	})
