...
app.post('/prod',function(req, res){
	...
	sql.connect(config, function(err){
		...
		var request = new sql.Request();
		request.input('input_parameters', sql.NVarChar, custcode)
		request.query(`SELECT TOP 1 * FROM dbo.RET_StockItem where (StockCode = @input_parameters or Barcode = @input_parameters) and (@input_parameters <> '')`, function(err, result){
			if(err) console.log(err)
				res.end(JSON.stringify(result));
		})
	})
...
app.post('/AuthReturns',function(req, res){
	...
	sql.connect(config, function(err){
		if(err) console.log(err);
		var request = new sql.Request();
		request.input('input_parameters', sql.Int, LID)
		request.query(`Select P.ID, P.CustomerCode, P.CustomerName, P.DateTime, Status_Desc from dbo.RET_ReturnStatus INNER JOIN (Select dbo.RET_ReturnsHeader.ID, dbo.RET_ReturnsHeader.LoginID, dbo.RET_Customer.CustomerCode, dbo.RET_Customer.CustomerName, dbo.RET_ReturnsHeader.DateTime, dbo.RET_ReturnsHeader.Status From dbo.RET_ReturnsHeader INNER JOIN dbo.RET_Customer ON dbo.RET_ReturnsHeader.CustomerID = dbo.RET_Customer.ID and status != 4) P ON dbo.RET_ReturnStatus.ID = P.Status where P.LoginID = @input_parameters`, function(err, result){
			if(err) console.log(err)
				res.end(JSON.stringify(result));
		})
	})
...
app.post('/status',function(req, res){
	...
	sql.connect(config, function(err){
		...
		request.query(`update dbo.RET_ReturnsHeader set Status = @input_parameters1 where ID = @input_parameters`, function(err, result){
			if(err) console.log(err)
				res.end(JSON.stringify(result));
		})
	})
...
app.post('/authrD',function(req, res){
	...
	sql.connect(config, function(err){
		...
		request.query(`update dbo.RET_ReturnDetail set Action = @input_parameters1, AuthorisedTime = @input_parameters2, Authorisenote = @input_parameters3 where ID = @input_parameters;`, function(err, result){
			if(err) console.log(err)
			console.log(result);
			res.end(JSON.stringify(result));
		})
